...
| `pfexp/techniques/proposals/extended_kalman.py` | particle_filter_tutorial `core/particle_filters/extended_kalman_particle_filter.py` | vectorized; log weights; resampling left to the experiment; **fixes:** matrix product in covariance prediction, variances instead of std, wrapped angles, prior centred on the prediction, stable angle Jacobian, position differences wrapped to the cyclic world (all switchable with `upstream_bugs`) |
