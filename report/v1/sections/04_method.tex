...
localization during the first steps. The learning experiments of \cref{sec:e11} also use a second setting, in which
the particles start around the true initial pose with a standard deviation of 0.2\,m and 0.2\,rad, so that the
filter tracks a robot whose starting point is roughly known.
...
\begin{itemize}
  ...
  \item \emph{Log likelihood}: $\sum_t \log \sum_i \tilde{\w}_t^{(i)}$ over a run, the estimate
        \eqref{eq:evidence} of $\log p(\z_{1:T} \mid \theta)$ under the noise settings the filter assumes. It is
        reported only for the motion-model proposal, whose weights take the form \eqref{eq:evidence} requires; the
        auxiliary and extended Kalman proposals weight in two stages and would need a different estimator.
